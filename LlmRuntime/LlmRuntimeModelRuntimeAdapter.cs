...
public sealed class LlmRuntimeModelRuntimeAdapter : ILmVsProxyModelRuntime, IDisposable
{
    ...
    private static LlmRuntimeOptions CopyOptions(LlmRuntimeOptions options)
    {
        return new LlmRuntimeOptions
        {
            ...
            RemoteVllmProfiles = options.RemoteVllmProfiles.Select(profile => new RemoteVllmProfile
            {
                Id = profile.Id,
                Name = profile.Name,
                ...
                Model = profile.Model,
                ModelCachePath = profile.ModelCachePath,
                ApiPort = profile.ApiPort,
                OpenAiBaseUrl = profile.OpenAiBaseUrl,
                TensorParallelSize = profile.TensorParallelSize,
                ContextLength = profile.ContextLength,
                MaxVramUsagePercent = profile.MaxVramUsagePercent,
                ExtraArguments = profile.ExtraArguments,
                Acceleration = profile.Acceleration,
                StartupTimeoutSeconds = profile.StartupTimeoutSeconds
            }).ToList(),
            SelectedRemoteVllmProfileId = options.SelectedRemoteVllmProfileId,
            RemoteVllmManager = options.RemoteVllmManager,
