...
**Current heirowLLM stream speed: 94.88 chars/sec** on `http://127.0.0.1:11436/api/chat-stream` as of `2026-05-29 07:05 America/Chicago`.

- Test artifact: `artifacts/stream-bench-heirowllm-11436-current-20260529.json`
...
Make heirowLLM/LlmRuntime responses stream at the rate the model infers tokens, across:

- Direct runtime: `http://127.0.0.1:11435/v1/chat/completions`
- Local heirowLLM web chat: `http://127.0.0.1:11436/api/chat-stream`
...
- Formats:
  ...
  - `HeirowLlmNdjson` for heirowLLM web chat.
...
| --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| Initial direct probe | Confirm runtime availability before patching | `artifacts/stream-bench-direct-11435.json` | `11435` | `Qwen3.5...` | n/a | 2,084 ms | 0 | 0 | 0.00 | Failed before headers; heirowLLM/runtime ports were down. |
...
| heirowLLM slow path | Reproduce local web chat bottleneck | `artifacts/stream-bench-heirowllm-11436-long-after.json` | `11436` | `Qwen3.5...` | 3,231 ms | 94,224 ms | 184 | 87 | 10.81 | Bottleneck reproduced. |
...
| heirowLLM after accounting fix | Verify usage/accounting coalescing | `artifacts/stream-bench-heirowllm-11436-long-after2.json` | `11436` | `Qwen3.5...` | 2,852 ms | 11,922 ms | 215 | 4 | 116.32 | Main bottleneck fixed. |
| heirowLLM repeat after accounting fix | Confirm result was repeatable | `artifacts/stream-bench-heirowllm-11436-long-after3.json` | `11436` | `Qwen3.5...` | 1,917 ms | 11,057 ms | 202 | 4 | 112.36 | Fix held on repeat. |
| Installed VS bridge | Measure installed bridge after heirowLLM core fix | `artifacts/stream-bench-vsbridge-11574-long-after2.json` | `11574` | `Qwen3.5...` | 6,065 ms | 19,301 ms | 213 | 0 | 77.14 | Still had high first-content latency. |
| Source bridge local path | Test bridge source patch using local heirowLLM | `artifacts/stream-bench-vsbridge-11576-localdirect-after2.json` | `11576` | `Qwen3.5...` | 2,293 ms | 19,345 ms | 222 | 0 | 64.63 | First-content latency improved sharply. |
...
| heirowLLM after low-latency/cache | Verify local web chat after final pass | `artifacts/stream-bench-heirowllm-11436-lowlatency-cache.json` | `11436` | `Qwen3.5...` | 2,841 ms | 14,294 ms | 224 | 4 | 73.43 | Usage frames stayed coalesced; model output varied. |
...
| Documented heirowLLM run | Persist model/goal fields in benchmark artifact | `artifacts/stream-bench-heirowllm-11436-documented.json` | `11436` | `Qwen3.5...` | 3,370 ms | 23,055 ms | 250 | 5 | 54.46 | Metadata capture verified; usage stayed low. |
...
| Current heirowLLM stream rate | Highlight live local web chat chars/sec at top of report | `artifacts/stream-bench-heirowllm-11436-current-20260529.json` | `11436` | `Qwen3.5...` | 1,853 ms | 11,286 ms | 240 | 4 | 94.88 | Current measured stream rate for the top callout. |

## Percent Changes

| Comparison | Total time change | Throughput change | First-content change | Notes |
| --- | ---: | ---: | ---: | --- |
| heirowLLM slow path -> heirowLLM after accounting fix | 87.3% decrease | 976.0% increase | 11.7% decrease | `94,224 ms` -> `11,922 ms`, `10.81` -> `116.32 chars/sec`. |
| heirowLLM slow path -> best heirowLLM repeat | 88.3% decrease | 939.4% increase | 40.7% decrease | `94,224 ms` -> `11,057 ms`; best stable post-fix repeat. |
| heirowLLM usage frames | 95.4% decrease | n/a | n/a | `87` usage frames -> `4`; this was the root event-amplification bottleneck. |
| Direct runtime repeat -> heirowLLM after accounting fix | 31.6% total overhead | 10.3% lower throughput | 195.9% later first content | Remaining proxy cost after the main fix. |
...
2. heirowLLM converted those into NDJSON deltas.
3. After nearly every content delta, heirowLLM also updated chat costs/session data and emitted a large `usage` event.
...
LM Studio stayed fast because it did not traverse this heirowLLM accounting/proxy path.

## Changes Made

- Added `tools/Benchmark-LlmStreamPipeline.ps1`.
  - Supports `OpenAiSse` and `HeirowLlmNdjson`.
...
- Coalesced heirowLLM chat usage telemetry in `SocketJack.LlmCore/Proxy/heirowLLM.cs`.
...
- Added dynamic caches in heirowLLM.
...
- Opted stream routes into low-latency mode.
  ...
  - `SocketJack.LlmCore/Proxy/heirowLLM.cs` `/api/chat-stream`
  - `SocketJack.LlmCore/Proxy/heirowLLM.cs` `/api/llm-client/chat-stream`
- Patched `SocketJack.CopilotMcpBridge/Program.cs`.
  - Defaults model discovery/chat stream forwarding to local heirowLLM `http://127.0.0.1:11436/` when available.
...
- `dotnet build .\heirowLLM\heirowLLM.csproj -c Debug --nologo -v:minimal` succeeded.
- `dotnet build .\SocketJack.CopilotMcpBridge\SocketJack.CopilotMcpBridge.csproj -c Debug --nologo -v:minimal` succeeded.
- Restarted rebuilt `heirowLLM.exe`; verified listeners on `11435` and `11436`.
- Ran direct runtime, heirowLLM, installed bridge, and source bridge streaming benchmarks.
...
- The reasoning model does not produce identical text across runs. For that reason, percent changes based on total time/chars/sec should be read together with frame counts. The frame count change is the strongest signal: heirowLLM usage telemetry dropped from 87 frames to 4-5 frames.
