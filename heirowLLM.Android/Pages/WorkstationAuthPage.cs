...
public sealed class WorkstationAuthPage : ContentPage
{
        ...
        string endpoint = "")
    {
        ...
        _status = new Label
        {
            Text = "Use an account stored by this heirowLLM Workstation. Your password is sent only to that Workstation.",
        ...
        Content = new ScrollView
        {
            Content = new VerticalStackLayout
            {
                ...
                Children =
                {
                    ...
                    new Label
                    {
                        Text = "Login or request a new account before heirowLLM Mobile can use chat, models, sessions, or PC Access.",
                        FontSize = 14,
    ...
    private async Task FinishAuthenticationAsync(string endpoint, WorkstationAuthResult result)
    {
        ...
        string normalizedEndpoint = HeirowLlmClient.NormalizeBaseUrl(endpoint);
        Uri endpointUri = new(normalizedEndpoint);
        ServerInfo? existing = _store.Load().FirstOrDefault(item =>
        {
            try
            {
                Uri saved = new(HeirowLlmClient.NormalizeBaseUrl(item.Endpoint));
        ...
        server.IsSaved = true;
        foreach (string credentialKey in HeirowLlmClient.CredentialKeys(server))
        ...
        _status.Text = "Signed in as " + result.Username + ". Opening heirowLLM Mobile...";
        await _authenticated(server);
    }

    private async Task RunAsync(string progress, Func<HeirowLlmClient, Task> action)
    {
        if (_busy) return;
        _busy = true;
        SetButtons(false);
    ...
    private bool TryReadFields(out string endpoint, out string username, out string password)
    {
        ...
        try { endpoint = HeirowLlmClient.NormalizeBaseUrl(_endpoint.Text ?? ""); }
