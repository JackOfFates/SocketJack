...
public sealed class ChatMessage : System.ComponentModel.INotifyPropertyChanged
{
    ...
    private string _workGoal = "";
    private string _workStatus = "working";
    private int _workProgressPercent;
    private string _workStepLabel = "";
    ...
    public string WorkGoal { get => _workGoal; set { if (_workGoal == value) return; _workGoal = value; PropertyChanged?.Invoke(this, new(nameof(WorkGoal))); PropertyChanged?.Invoke(this, new(nameof(HasWorkGoal))); } }
    public string WorkStatus { get => _workStatus; set { if (_workStatus == value) return; _workStatus = value; PropertyChanged?.Invoke(this, new(nameof(WorkStatus))); PropertyChanged?.Invoke(this, new(nameof(WorkStatusLabel))); } }
    public int WorkProgressPercent { get => _workProgressPercent; set { int next = Math.Clamp(value, 0, 100); if (_workProgressPercent == next) return; _workProgressPercent = next; PropertyChanged?.Invoke(this, new(nameof(WorkProgressPercent))); PropertyChanged?.Invoke(this, new(nameof(WorkProgress))); PropertyChanged?.Invoke(this, new(nameof(WorkProgressLabel))); } }
    public string WorkStepLabel { get => _workStepLabel; set { if (_workStepLabel == value) return; _workStepLabel = value; PropertyChanged?.Invoke(this, new(nameof(WorkStepLabel))); } }
    ...
    public ObservableCollection<JackhammerPlanStep> JackhammerSteps { get; } = new();
    ...
    public bool HasWorkGoal => !string.IsNullOrWhiteSpace(WorkGoal);
    public double WorkProgress => WorkProgressPercent / 100d;
    public string WorkProgressLabel => $"{WorkProgressPercent}%";
    public string WorkStatusLabel => WorkStatus.Replace('_', ' ').ToUpperInvariant();
...
public sealed class JackhammerPlanStep
{
    ...
    public string StepLabel => $"STEP {Number}/{Math.Max(1, Total)} · {Status.Replace('_', ' ').ToUpperInvariant()}";
    public string NumberLabel => Number.ToString();
    public Color AccentColor => Status switch
    {
        "completed" => Color.FromArgb("#34D399"),
        "in_progress" => Color.FromArgb("#60A5FA"),
        "blocked" => Color.FromArgb("#FB7185"),
        _ => Color.FromArgb("#94A3B8")
    };
    public Color SurfaceColor => Status switch
    {
        "completed" => Color.FromArgb("#142F2A"),
        "in_progress" => Color.FromArgb("#172A4C"),
        "blocked" => Color.FromArgb("#3A1C28"),
        _ => Color.FromArgb("#182234")
    };
}
...
public sealed class ChatStreamEvent
{
    ...
    public string JackhammerGoal { get; init; } = "";
    public string JackhammerStatus { get; init; } = "";
    public int JackhammerProgressPercent { get; init; } = -1;
