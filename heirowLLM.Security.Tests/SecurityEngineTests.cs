...
public sealed class SecurityEngineTests {
    ...
    public void Initialize() {
        ...
        _root = Path.Combine(Path.GetTempPath(), "heirowLLM.Security.Tests", id);
        _registryPath = @"Software\SocketJack\heirowLLM\Security\Tests\" + id;
    ...
    public void PortableRecoveryEnrollsAReplacementMachineStore() {
        ...
        string replacementRoot = Path.Combine(Path.GetTempPath(), "heirowLLM.Security.Tests", replacementId);
        string replacementRegistry = @"Software\SocketJack\heirowLLM\Security\Tests\" + replacementId;
