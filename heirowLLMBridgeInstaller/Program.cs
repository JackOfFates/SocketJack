...
namespace heirowLLMBridgeInstaller;

internal static class Program {
    private const string ProductName = "heirowLLM";
    private const string Publisher = "SocketJack";
    private const string MainExecutableName = "heirowLLM.exe";
    private const string StartupValueName = "heirowLLM";
    ...
    private const string ProductRegistryPath = @"Software\SocketJack\heirowLLM";
    private const string RunRegistryPath = @"Software\Microsoft\Windows\CurrentVersion\Run";
    private const string AppPathsRegistryPath = @"Software\Microsoft\Windows\CurrentVersion\App Paths\heirowLLM.exe";
    private const string UninstallRegistryPath = @"Software\Microsoft\Windows\CurrentVersion\Uninstall\heirowLLM";
    private const int MaxParallelDownloads = 6;
    private const int DownloadBufferSize = 1024 * 1024;
    private const string EmbeddedMsiResourceName = "heirowLLM-Setup.msi";
    private const string SecurityBrokerExecutableName = "heirowLLM.SecurityBroker.exe";
    private const string SecurityBrokerServiceName = "heirowLLMSecurityBroker";
    ...
    private static void PrintHelp() {
        ...
        Console.WriteLine("--install-dir <path> Install directory. Default: %ProgramFiles%\\heirowLLM");
    ...
    private static async Task InstallAsync(InstallerOptions options) {
        ...
        string tempRoot = Path.Combine(Path.GetTempPath(), "heirowLLMBridgeInstaller-" + Guid.NewGuid().ToString("N"));
        ...
        try {
            ...
            if (!File.Exists(mainExe))
                throw new InvalidOperationException(MainExecutableName + " was not installed. Verify the update folder contains the heirowLLM executable.");
    ...
    private static async Task<bool> TryInstallPackagedMsiAsync(InstallerOptions options) {
        string msiPath = Path.Combine(AppContext.BaseDirectory, "heirowLLM-Setup.msi");
        ...
        if (!File.Exists(msiPath)) {
            ...
            extractionRoot = Path.Combine(Path.GetTempPath(), "heirowLLMInstaller-" + Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(extractionRoot);
            msiPath = Path.Combine(extractionRoot, "heirowLLM-Setup.msi");
    ...
    private static void WriteDefaultGuiSettings(InstallerOptions options) {
        string settingsRoot = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SocketJack", "heirowLLM");
        Directory.CreateDirectory(settingsRoot);
        string settingsPath = Path.Combine(settingsRoot, "heirowLLM.settings.json");
    ...
    private static void RecordLegacyInstallRoot(string fallbackRoot) {
        ...
        foreach (Process process in Process.GetProcessesByName("heirowLLM")) {
            try {
        ...
        string userRoot = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SocketJack", "heirowLLM");
    ...
    private static void StopStaleSecurityBrokerProcesses() {
        foreach (Process process in Process.GetProcessesByName("heirowLLM.SecurityBroker")) {
            try {
    ...
    private static void EnsureSecurityBrokerServiceInstalled(string installDirectory) {
        ...
        if (!File.Exists(brokerPath)) {
            throw new InvalidOperationException(
                SecurityBrokerExecutableName + " was not installed. The update payload is incomplete and heirowLLM cannot securely start.");
        }
        ...
        ScResult config = RunServiceControl(
            ...
            "start=", "auto",
            "obj=", "LocalSystem",
            "DisplayName=", "heirowLLM Security Broker");
        if (config.ExitCode == 1060) {
            ...
            if (create.ExitCode != 0)
                throw new InvalidOperationException("Windows could not install the heirowLLM Security Broker service: " + create.Output);
        } else if (config.ExitCode != 0) {
            throw new InvalidOperationException("Windows could not configure the heirowLLM Security Broker service: " + config.Output);
        }

        _ = RunServiceControl("description", SecurityBrokerServiceName, "Protects heirowLLM workstation enrollment, TPM, Windows Hello, and hardware identity operations.");
        ScResult start = RunServiceControl("start", SecurityBrokerServiceName);
        if (start.ExitCode != 0 && start.ExitCode != 1056)
            throw new InvalidOperationException("Windows could not start the heirowLLM Security Broker service: " + start.Output);
        ...
        throw new InvalidOperationException("The heirowLLM Security Broker service was installed but did not reach the running state.");
    }
    ...
    private static bool CanRemoveInstallDirectory(string path) {
        ...
        return !string.IsNullOrWhiteSpace(path) &&
               ...
               (path.EndsWith("heirowLLM", StringComparison.OrdinalIgnoreCase) ||
                path.EndsWith("LMVS Bridge", StringComparison.OrdinalIgnoreCase));
    }

    private static string BuildHardwarePreflightReport() {
        var lines = new List<string> { "heirowLLM checked this PC before requesting administrator access.", "" };
        ...
        lines.Add("All user data will be kept under %LOCALAPPDATA%\\SocketJack\\heirowLLM.");
...
internal sealed record InstallerOptions {
    ...
    public string InstallDirectory { get; init; } = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "heirowLLM");
