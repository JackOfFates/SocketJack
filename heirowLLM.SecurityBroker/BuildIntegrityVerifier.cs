...
namespace heirowLLM.SecurityBroker;
...
public sealed class BuildIntegrityVerifier {
    public BuildIntegrityResult Verify(uint processId, bool development, bool allowHashStampedLocalRelease = false) {
        ...
        if (string.IsNullOrWhiteSpace(path) || !string.Equals(Path.GetFileName(path), "heirowLLM.exe", StringComparison.OrdinalIgnoreCase))
            return new(false, "Only heirowLLM.exe may request workstation unlocks.");
        if (development)
        ...
        using RegistryKey? config = Registry.LocalMachine.OpenSubKey(@"Software\SocketJack\heirowLLM\Security");
        ...
        if (expectedPublisher.Length != 64 || string.IsNullOrWhiteSpace(manifestPublicKey))
            return new(false, "Official signing is not configured. Install a signed heirowLLM package with publisher and manifest keys.");
        if (!AuthenticodeTrust.IsTrusted(path)) return new(false, "The heirowLLM executable has no valid Authenticode signature.");
        try {
            ...
            if (!CryptographicOperations.FixedTimeEquals(Convert.FromHexString(actualPublisher), Convert.FromHexString(expectedPublisher)))
                return new(false, "The heirowLLM executable was signed by an untrusted publisher.");
        } catch (Exception ex) { return new(false, "Cannot validate the heirowLLM publisher: " + ex.Message); }
        ...
        try {
            ...
            if (entry == null) return new(false, "heirowLLM.exe is not listed in the signed release manifest.");
            ...
            if (expectedHash.Length != 64 || !CryptographicOperations.FixedTimeEquals(Convert.FromHexString(actualHash), Convert.FromHexString(expectedHash)))
                return new(false, "heirowLLM.exe does not match the signed release manifest.");
    ...
    private static BuildIntegrityResult VerifyHashStampedLocalRelease(string path) {
        ...
        if (!File.Exists(markerPath))
            return new(false, "The secure Release hash marker is missing. Rebuild heirowLLM in Release mode.");
        try {
            ...
            if (expectedHash.Length != 64 ||
                ...
                return new(false, "heirowLLM.exe does not match the hash-stamped Release build.");
