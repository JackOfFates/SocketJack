...
| TypeScript clients | Available | `HttpServer.GenerateTypeScriptClient` emits dependency-free typed route methods, request aliases, path/query/header handling, HTTP errors, and WebSocket message interfaces. |
...
| SockJackDml workflow tools | Complete | `sockjackdml_plan_create`, `sockjackdml_progress_document_create`, and `sockjackdml_plan_execute` are available through heirowLLM Agent tools and local API routes. |
...
## heirowLLM Chat And Session Management
...
| WPF session controls | Planned | Matching session actions, search, and progress controls are planned for heirowLLM WPF. |
...
| HTTP access logs | Complete | Default-on daily JSONL-style access logs are written under `C:\heirowLLM\Logs`. |
...
- `SocketJack`, `heirowLLM`, `heirowLLMCompanion`, `LlmRuntime`, and update projects have passing build records in the progress logs.
...
- Complete the heirowLLM session-management refresh: save, lock/unlock, clone, regex search, token progress, WPF controls, and metadata compatibility.
...
- Run a live Web Chat Agent-mode smoke test for SockJackDml workflow tools after launching heirowLLM.
