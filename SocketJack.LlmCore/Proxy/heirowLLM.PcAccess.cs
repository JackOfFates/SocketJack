...
public partial class HeirowLlm
{
    ...
    private string HandlePcAccessFtp(NetworkConnection connection, HttpRequest request)
    {
        ...
        if (!ftp.Running)
            return BuildJsonError(request, 409, "FTP Stopped", "Start FTP for this user in heirowLLM Workstation before opening PC Access files.");
        if (!ftp.OwnerMatches)
        ...
        if (string.IsNullOrWhiteSpace(ftp.UserName) || string.IsNullOrWhiteSpace(ftp.Password))
            return BuildJsonError(request, 409, "FTP Credentials Missing", "Configure an FTP username and password in heirowLLM Workstation.");
