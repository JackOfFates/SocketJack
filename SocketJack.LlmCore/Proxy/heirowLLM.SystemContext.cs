...
namespace SocketJack.Net
{
    public partial class HeirowLlm
    {
        ...
        private string BuildJackCapabilityContextSystemHint(ChatPermissionState permissions, bool consentUi)
        {
            ...
            return "[heirow capability context] Enabled: " + (enabled.Count == 0 ? "none" : string.Join(", ", enabled)) +
        ...
        private static void WriteSystemContextToolSchemas(HeirowLlm proxy, Utf8JsonWriter writer)
        {
            proxy.WriteProxyResearchToolSchema(writer, RunningApplicationsToolName,
                "List running Windows applications as bounded read-only context. If standing access is disabled, heirow asks the user before this call runs.",
            ...
            proxy.WriteProxyResearchToolSchema(writer, WindowsServicesToolName,
                "List Windows services as bounded read-only context. If standing access is disabled, heirow asks the user before this call runs.",
                Array.Empty<string>(), new[]
                {
            ...
            proxy.WriteProxyResearchToolSchema(writer, InspectFilesToolName,
                "Read, list, or search files as read-only heirow context. Every unchecked call asks the user first. Existing session-root and approved-root restrictions still apply; this tool cannot change files.",
                new[] { "action" }, new[]
                {
