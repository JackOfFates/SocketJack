...
public partial class HeirowLlm
{
    ...
    private string _dreamExecutionModel = "";
    private bool _dreamExecutionModelConfigured;
    internal Func<string, string> DreamReflectionOverrideForDiagnostics { get; set; }

    public void SetDreamExecutionModelDiagnostics(string model)
    {
        lock (_dreamLock)
        {
            _dreamExecutionModel = string.IsNullOrWhiteSpace(model) ? "auto" : model.Trim();
            _dreamExecutionModelConfigured = true;
        }
    }

    public string GetDreamExecutionModelDiagnostics()
    {
        lock (_dreamLock)
            return _dreamExecutionModelConfigured && !string.IsNullOrWhiteSpace(_dreamExecutionModel) ? _dreamExecutionModel : "auto";
    }
    ...
    {
        DreamSettings settings = EffectiveDreamSettings(state);
        string configuredModel = ResolveDreamExecutionModel(settings.model);
        bool gateHeld = false;
        ...
        try
        {
            ...
            if (DreamReflectionOverrideForDiagnostics != null)
            {
                ...
                entry.dreamModel = FirstNonEmpty(configuredModel, "diagnostic-model");
            ...
            else
            {
                ...
                entry.dreamModel = ResolveDreamCompletionModel(completion, configuredModel);
            ...
            entry.checksAndBalancesModel = ResolveChecksAndBalancesModel(transcript.SelectedModel, entry.dreamModel, configuredModel);
            entry.checksAndBalancesStatus = "running";
            state.phase = "checks-and-balances"; state.progress = 88;
            AlignmentRunResult checks = await RunChecksAndBalancesForDreamAsync(state.ownerKey, entry.id, entry.checksAndBalancesData, entry.processedMessages, entry.checksAndBalancesModel, entry.dreamService, timeout.Token, entry.dreamModel, configuredModel).ConfigureAwait(false);
            if (!checks.Success)
            {
    ...
    {
        string configuredModel = ResolveDreamExecutionModel(settings.model);
        entry.checksAndBalancesModel = ResolveChecksAndBalancesModel("", entry.dreamModel, configuredModel);
        AlignmentRunResult result = await RunChecksAndBalancesForDreamAsync(state.ownerKey, entry.id, entry.checksAndBalancesData, entry.processedMessages, entry.checksAndBalancesModel, entry.dreamService, token, entry.dreamModel, configuredModel).ConfigureAwait(false);
    ...
    private string ResolveDreamExecutionModel(string ownerModel)
    {
        string applicationModel;
        lock (_dreamLock)
            applicationModel = _dreamExecutionModelConfigured ? _dreamExecutionModel : "";
        return FirstNonEmpty(applicationModel, ownerModel, "auto");
    }
    ...
    private void EnrichRuntimeRequestWithMemories(NetworkConnection connection, HttpRequest request, bool responsesApi)
    {
        try
        {
            string ownerKey = GetChatSessionOwnerKey(connection, request); ProcessExplicitChatMemoryCommands(ownerKey, "runtime-shared", request.Body); string hint = BuildChatMemorySystemHint(ownerKey, ExtractLatestChatUiUserText(request.Body)); if (string.IsNullOrWhiteSpace(hint)) return;
    ...
    private string LegacyDreamStatePath => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "SocketJack", "heirowLLM", "dream-state.json");
