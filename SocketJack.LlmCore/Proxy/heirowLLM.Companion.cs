    ...
    public partial class HeirowLlm
    {
        ...
        private string ExecuteCompanionAction(string ownerKey, HttpRequest request, bool observationApprovedOnce)
        {
            ...
            try
            {
                ...
                bool controlsDesktop = type is "move" or "click" or "key" or "shortcut" or "text" or "scroll" or "clipboard" or "launch" or "terminal";
                if (controlsDesktop)
            ...
            try
            {
                ...
                foreach (JsonProperty property in document.RootElement.EnumerateObject())
                {
                    if (property.NameEquals("tools") && property.Value.ValueKind == JsonValueKind.Array)
                    {
                        ...
                        {
                            // Companion is a deliberately narrow desktop-control mode. Do not
                            // leave Agent, terminal-process, Browser, or coordination schemas in
                            // the request: a GUI launched by run_command_in_terminal is not a
                            // TerminalService tracked process, and following it with
                            // terminal_process_status produces a false owner/process failure.
                            string existingName = ExtractToolSchemaName(tool) ?? "";
                            if (string.IsNullOrWhiteSpace(existingName) ||
                                !IsProxyOwnedResearchTool(existingName))
                                tool.WriteTo(writer);
                        }
                    ...
                    else if (property.NameEquals("messages") && property.Value.ValueKind == JsonValueKind.Array)
                    {
                        ...
                        {
                            string role = CompanionJsonString(message, "role");
                            string content = CompanionJsonString(message, "content");
                            if (!role.Equals("system", StringComparison.OrdinalIgnoreCase) ||
                                content.Contains("[heirowLLM memory context and policy]", StringComparison.OrdinalIgnoreCase))
                                message.WriteTo(writer);
                        }
                        writer.WriteStartObject();
                        writer.WriteString("role", "system");
                        writer.WriteString("content", "Companion desktop workflow: use only companion_action. Never use VS, terminal process, or browser tools to launch or verify a Windows GUI. For a named app, call launch and require success, then call observe and visually confirm its window before manipulating it. Complete every requested step in order, one action at a time, using text plus shortcuts such as CTRL+S and ENTER or move/click when needed. Call observe after state-changing actions when confirmation is needed. Use screen to take the requested screenshot of the foreground application window; use observe for full-desktop pixels needed to decide the next action. A screenshot is evidence, not the end of the task: continue through later save or confirmation steps. Every action still requires its permission; denials are final and confirmations cannot be self-approved.");
                        writer.WriteEndObject();
                        writer.WriteEndArray();
                    }
                    else if (property.NameEquals("max_tokens") || property.NameEquals("max_completion_tokens") || property.NameEquals("max_output_tokens"))
                    {
                        int requested = property.Value.ValueKind == JsonValueKind.Number && property.Value.TryGetInt32(out int parsed)
                            ? parsed
                            : 256;
                        writer.WriteNumber(property.Name, Math.Clamp(requested, 1, 256));
                        wroteMaxTokens = true;
                    }
                    else
                    {
                ...
                if (!wroteMessages)
                {
                    ...
                    writer.WriteStartObject();
                    writer.WriteString("role", "system");
                    writer.WriteString("content", "Companion desktop workflow: use only companion_action; launch then observe to confirm, manipulate with permitted keyboard or pointer actions, use screen for a requested foreground-window screenshot, and continue until every requested save and verification step is complete. Honor every denial.");
                    writer.WriteEndObject();
                    writer.WriteEndArray();
                }
                if (!wroteMaxTokens)
                    writer.WriteNumber("max_tokens", 256);
                writer.WriteEndObject();
                writer.Flush();
        ...
        private void WriteCompanionToolSchema(Utf8JsonWriter writer, ChatPermissionState permissions)
        {
            ...
            if (permissions?.companionCursorControl == true)
                allowedActions.AddRange(new[] { "move", "click" });
            ...
            WriteProxyResearchToolSchema(
                writer,
                "companion_action",
                "Operate Windows through permissioned Companion actions. Launch named apps, observe to confirm visible state, manipulate the foreground window with keyboard or pointer input, use screen for a requested foreground-window screenshot, and complete all requested steps.",
                new[] { "type" },
                new[]
                {
                    new ProxyToolParameter("type", "string", "Allowed: " + allowedActionText + ". observe returns the full desktop for visual decisions; screen captures the foreground application window as requested evidence."),
                    new ProxyToolParameter("value", "string", "Application target for launch, key chord for shortcut, or terminal command. For type=text or clipboard, put the exact content in text."),
                    ...
                    new ProxyToolParameter("keyCode", "integer", "Windows virtual key."),
                    new ProxyToolParameter("down", "boolean", "Key-down state."),
                    new ProxyToolParameter("text", "string", "Text to type or copy."),
