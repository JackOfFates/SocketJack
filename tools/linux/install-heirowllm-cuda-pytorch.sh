...
usage() {
  cat <<'EOF'
Usage: install-heirowllm-cuda-pytorch.sh [options]

Installs the Linux Python/PyTorch/CUDA runtime used by heirowLLM Workstation.

Options:
  --app-root PATH         heirowLLM Workstation output root. Defaults to two levels above this script.
...
log() {
  printf '[heirowllm-linux-install] %s\n' "$*" >&2
}
...
if [ -z "$APP_ROOT" ]; then
  if [ -n "${JACKONNX_PYTHON_HOME:-}" ]; then
    ...
    if [ "${APP_ROOT#/opt/}" != "$APP_ROOT" ] && [ ! -w "$APP_ROOT" ]; then
      APP_ROOT="${HEIROWLLM_DATA_ROOT:-/var/lib/heirowllm}"
    fi
  fi
fi
...
resolve_python() {
  ...
  mkdir -p "$APP_ROOT"
  log "Creating heirowLLM Linux Python venv at $APP_ROOT/Python"
