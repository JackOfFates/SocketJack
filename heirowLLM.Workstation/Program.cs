...
namespace heirowLLM.Workstation;
...
internal static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("heirowLLM Workstation option error: " + ex.Message);
        ...
        try
        {
            ...
            Console.WriteLine("Press Ctrl+C to stop heirowLLM Workstation.");
        ...
        {
            Console.Error.WriteLine("heirowLLM Workstation failed: " + ex.Message);
            if (options.Verbose)
    ...
    private sealed class WorkstationHost : IDisposable
    {
        ...
        private readonly HttpHeirowLlmModelRuntime _lmStudioRuntime;
        ...
        {
            _options = options;
            _proxy = new HeirowLlm(options.LmStudioHost, options.LmStudioPort, options.ProxyPort, options.ChatServerPort, options.DataRoot)
            {
            ...
            _lmStudioRuntime = new HttpHeirowLlmModelRuntime("LM Studio", options.LmStudioBaseUrl, () => Task.CompletedTask);
            ...
            _proxy.HeirowSongMediaExecutor = new JackOnnxHeirowSongMediaExecutor(_jackOnnxRuntime);
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (!_proxy.ChatServer.IsListening && !_proxy.ChatServer.Listen())
                throw new InvalidOperationException("Unable to start heirowLLM web console on port " + _options.ChatServerPort.ToString(CultureInfo.InvariantCulture) + ".");

            if (_options.EnableSqlAdmin)
                EnableSqlAdminPanel();

            _started = true;
            Log("heirowLLM Workstation is running.");
        ...
        private void ConfigureModelRuntime()
        {
            ...
            Log("heirowONNX tools registered with LlmRuntime: " + _jackOnnxToolDefinitions.Count.ToString(CultureInfo.InvariantCulture) + ".");
        }
        ...
        {
            Console.WriteLine("[heirowLLM.Workstation] " + message);
        }

        public void Dispose()
        {
            if (_started)
                Log("Stopping heirowLLM Workstation.");
    ...
    private sealed class WorkstationOptions
    {
        ...
        public int ProxyPort { get; private set; } = ReadPort("HEIROWLLM_PROXY_PORT", DefaultProxyPort);
        public int CopilotDuplicatorPort { get; private set; } = ReadPort("HEIROWLLM_COPILOT_DUPLICATOR_PORT", HeirowLlm.DefaultCopilotDuplicatorPort);
        public int ChatServerPort { get; private set; } = ReadPort("HEIROWLLM_CHAT_PORT", DefaultChatServerPort);
        public int ModelRuntimePort { get; private set; } = ReadPort("HEIROWLLM_MODEL_RUNTIME_PORT", DefaultModelRuntimePort);
        public string LmStudioHost { get; private set; } = "127.0.0.1";
        public int LmStudioPort { get; private set; } = ReadPort("HEIROWLLM_LMSTUDIO_PORT", DefaultLmStudioPort);
        public string ChatModel { get; private set; } = Environment.GetEnvironmentVariable("HEIROWLLM_CHAT_MODEL") ?? "lm-studio";
        public bool StartEmbeddedRuntime { get; private set; } = ReadBool("HEIROWLLM_START_EMBEDDED_RUNTIME", true);
        public bool RestoreLoadedModelsOnStartup { get; private set; } = ReadBool("HEIROWLLM_RESTORE_LOADED_MODELS_ON_STARTUP", false);
        public bool EnableLmStudioFallback { get; private set; } = ReadBool("HEIROWLLM_LMSTUDIO_FALLBACK", true);
        public bool IncludeLmStudioModels { get; private set; } = ReadBool("HEIROWLLM_INCLUDE_LMSTUDIO_MODELS", true);
        public bool WebChatModelLoadApiEnabled { get; private set; } = ReadBool("HEIROWLLM_WEBCHAT_MODEL_LOAD", false);
        public bool CopilotDuplicatorEnabled { get; private set; } = ReadBool("HEIROWLLM_COPILOT_DUPLICATOR", true);
        public bool PublicAccessEnabled { get; private set; } = ReadBool("HEIROWLLM_PUBLIC_ACCESS", false);
        public bool MobileAccessEnabled { get; private set; } = ReadBool("HEIROWLLM_MOBILE_ACCESS", false);
        public bool BrowserPrivateNetworkAccessEnabled { get; private set; } = ReadBool("HEIROWLLM_BROWSER_PRIVATE_NETWORK_ACCESS", false);
        public bool AuthTokensInQueryEnabled { get; private set; } = ReadBool("HEIROWLLM_AUTH_TOKENS_IN_QUERY", false);
        public bool UseSocketJackMasterAuth { get; private set; } = ReadBool("HEIROWLLM_SOCKETJACK_AUTH", true);
        public bool StoreLocalWebAuthAccounts { get; private set; } = ReadBool("HEIROWLLM_STORE_LOCAL_WEB_AUTH", false);
        public bool EnableSqlAdmin { get; private set; } = ReadBool("HEIROWLLM_SQL_ADMIN", false);
        public int PromptTimeoutMinutes { get; private set; } = ReadPort("HEIROWLLM_PROMPT_TIMEOUT_MINUTES", 30);
        public string ControlAuthToken { get; private set; } = Environment.GetEnvironmentVariable("LLMRUNTIME_CONTROL_TOKEN") ?? "";
        public string DataRoot { get; private set; } = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".heirowllm");
        ...
        public static WorkstationOptions Parse(string[] args)
        {
            ...
            options.ApplyLmStudioUrl(Environment.GetEnvironmentVariable("HEIROWLLM_LMSTUDIO_URL"), required: false);
        ...
        {
            writer.WriteLine("heirowLLM Workstation cross-platform host");
            writer.WriteLine();
            writer.WriteLine("Usage:");
            writer.WriteLine("  dotnet run --project ./heirowLLM.Workstation -- [options]");
            writer.WriteLine();
            ...
            writer.WriteLine("  --data-root PATH                    Runtime config root. Default: ~/.heirowllm");
            ...
            writer.WriteLine("  --mobile-access true|false          Enable short-lived pairing for heirowLLM Mobile. Default: false.");
        ...
        {
            DataRoot = FullPath(Environment.GetEnvironmentVariable("HEIROWLLM_DATA_ROOT") ?? DataRoot, "HEIROWLLM_DATA_ROOT");
            WorkspaceRoot = FullPath(Environment.GetEnvironmentVariable("HEIROWLLM_WORKSPACE_ROOT") ?? WorkspaceRoot, "HEIROWLLM_WORKSPACE_ROOT");
            string contentRoot = Environment.GetEnvironmentVariable("HEIROWLLM_CONTENT_ROOT") ?? Environment.CurrentDirectory;
            string modelsLocation = Environment.GetEnvironmentVariable("HEIROWLLM_MODELS_LOCATION") ?? "";
            ...
            ModelRoot = FullPath(Environment.GetEnvironmentVariable("HEIROWLLM_MODEL_ROOT") ?? defaultModelRoot, "HEIROWLLM_MODEL_ROOT");
            CompleteModelRoot = FullPath(Environment.GetEnvironmentVariable("HEIROWLLM_COMPLETE_MODEL_ROOT") ?? defaultCompleteModelRoot, "HEIROWLLM_COMPLETE_MODEL_ROOT");
            ToolRoot = FullPath(Environment.GetEnvironmentVariable("HEIROWLLM_TOOL_ROOT") ?? Path.Combine(contentRoot, "Tools"), "HEIROWLLM_TOOL_ROOT");
            AgentRoot = FullPath(Environment.GetEnvironmentVariable("HEIROWLLM_AGENT_ROOT") ?? Path.Combine(contentRoot, "Agents"), "HEIROWLLM_AGENT_ROOT");

            string provider = Environment.GetEnvironmentVariable("HEIROWLLM_RUNTIME_PROVIDER") ?? "";
