...
internal static class LinuxDesktopRemoteControl
{
    ...
    public static void TryRegister(Action<string> log)
    {
        ...
        lock (SyncRoot)
        {
            HeirowLlmRemoteControl.ProviderName = "SocketJack.Linux.OpenCL/X11";
            HeirowLlmRemoteControl.CaptureScreen = CaptureScreen;
            HeirowLlmRemoteControl.ExecuteInput = ExecuteInput;
        }
    ...
    private static HeirowLlmScreenCaptureResult CaptureScreen(HeirowLlmScreenCaptureOptions options)
    {
        options ??= new HeirowLlmScreenCaptureOptions();
        ...
        bool tryOpenCl = probe.OpenClAvailable && string.Equals(
            Environment.GetEnvironmentVariable("HEIROWLLM_CAPTURE_OPENCL") ?? "true",
            "true",
        ...
        return new HeirowLlmScreenCaptureResult
        {
            Bytes = attempt.Bytes,
            MimeType = "image/jpeg",
            Width = output.Width,
    ...
    private static HeirowLlmRemoteInputResult ExecuteInput(HeirowLlmRemoteInputRequest input)
    {
        if (!CommandExists("xdotool"))
            throw new InvalidOperationException("xdotool is required for Linux desktop remote input.");

        input ??= new HeirowLlmRemoteInputRequest();
        DesktopProbe probe = ResolveDesktop();
        ...
        return new HeirowLlmRemoteInputResult
        {
            Action = action,
            X = start.X,
            Y = start.Y,
    ...
    private static DesktopGeometry ApplyOutputBounds(DesktopGeometry source, HeirowLlmScreenCaptureOptions options)
    {
    ...
    private static DesktopProbe ResolveDesktop()
    {
        ...
        string xauthority = FirstNonEmpty(
            Environment.GetEnvironmentVariable("HEIROWLLM_CAPTURE_XAUTHORITY"),
        ...
        AddDisplayCandidate(displays, Environment.GetEnvironmentVariable("HEIROWLLM_CAPTURE_DISPLAY"));
        ...
        string display = displays.FirstOrDefault(candidate => ProbeDisplay(candidate, xauthority)) ??
            throw new InvalidOperationException("No accessible Xorg display was found. Set HEIROWLLM_CAPTURE_DISPLAY and HEIROWLLM_CAPTURE_XAUTHORITY.");
    ...
    {
        string title = FirstNonEmpty(Environment.GetEnvironmentVariable("HEIROWLLM_CAPTURE_WINDOW_TITLE"), "heirowLLM Workstation");
        if (!CommandExists("xdotool"))
            return null;
