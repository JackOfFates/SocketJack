...
[TestClass]
public sealed class HeirowLlmPersistenceTests
{
    [TestMethod]
    public void LegacyThirtySevenColumnPermissionRowsKeepExistingIndexes()
    {
        string dataRoot = Path.Combine(Path.GetTempPath(), "heirowllm-legacy-permission-row-" + Guid.NewGuid().ToString("N"));
        try
        {
            ...
            row[11] = "true";
            row[36] = "true";
            var method = typeof(HeirowLlm).GetMethod("ChatPermissionStateFromRow", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic);
    ...
    {
        string dataRoot = Path.Combine(Path.GetTempPath(), "heirowllm-proxy-persistence-" + Guid.NewGuid().ToString("N"));
        ...
        try
        {
            ...
            using (var second = CreateProxy(dataRoot))
            {
                ...
                Assert.IsTrue(File.Exists(Path.Combine(dataRoot, "SocketJack", "heirowLLMChat", "SocketJackDatabase.json")));
            }
        }
        finally
        {
    ...
    {
        string dataRoot = Path.Combine(Path.GetTempPath(), "heirowllm-companion-permissions-" + Guid.NewGuid().ToString("N"));
        try
        {
            using (var first = CreateProxy(dataRoot))
            {
    ...
    {
        string dataRoot = Path.Combine(Path.GetTempPath(), "heirowllm-dream-permissions-" + Guid.NewGuid().ToString("N"));
        try
        {
            using var proxy = CreateProxy(dataRoot);
    ...
    {
        string dataRoot = Path.Combine(Path.GetTempPath(), "heirowllm-dream-settings-" + Guid.NewGuid().ToString("N"));
        try
        {
            using (var proxy = CreateProxy(dataRoot))
            {
    ...
    {
        string dataRoot = Path.Combine(Path.GetTempPath(), "heirowllm-dream-hardware-" + Guid.NewGuid().ToString("N"));
        try
        {
    ...
    {
        string dataRoot = Path.Combine(Path.GetTempPath(), "heirowllm-proxy-local-permissions-" + Guid.NewGuid().ToString("N"));
    ...
    private static HeirowLlm CreateProxy(string dataRoot)
    {
        return new HeirowLlm("127.0.0.1", 11434, 11435, 0, dataRoot);
    }
}
