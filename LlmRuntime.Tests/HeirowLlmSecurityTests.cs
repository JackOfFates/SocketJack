...
[TestClass]
public sealed class HeirowLlmSecurityTests
{
    [TestMethod]
    public void CompanionPolicyDetectsFinancialAndSensitiveActions()
    {
        Assert.IsTrue(SocketJack.Net.HeirowLlm.IsCompanionFinancialAction("open checkout and purchase the subscription"));
        Assert.IsTrue(SocketJack.Net.HeirowLlm.IsCompanionFinancialAction("Invoke-RestMethod https://api.stripe.com/v1/payment_intents"));
        Assert.IsTrue(SocketJack.Net.HeirowLlm.IsCompanionFinancialAction("transfer bitcoin to this wallet"));
        Assert.IsFalse(SocketJack.Net.HeirowLlm.IsCompanionFinancialAction("open Notepad and type a draft"));
        Assert.IsTrue(SocketJack.Net.HeirowLlm.IsCompanionSensitiveText("api_key=sk-example"));
        Assert.IsTrue(SocketJack.Net.HeirowLlm.IsCompanionSensitiveText("password: hunter2"));
        Assert.IsFalse(SocketJack.Net.HeirowLlm.IsCompanionSensitiveText("the user prefers dark mode"));
        StringAssert.Contains(SocketJack.Net.HeirowLlm.RedactCompanionSensitiveText("password=hunter2"), "[redacted]");
    }

    [TestMethod]
    public void CompanionObservationIsConvertedToFreshVisionInput()
    {
        MethodInfo? extract = typeof(HeirowLlm).GetMethod(
        ...
        string screenshotOnly = JsonSerializer.Serialize(new
        {
            ok = true,
            type = "screen",
            requestedType = "screen",
            contentType = "image/jpeg",
            data = jpegBase64
        });
        Assert.AreEqual("data:image/jpeg;base64," + jpegBase64,
            (string)extract.Invoke(null, ["companion_action", screenshotOnly])!);
    }

    [TestMethod]
    public void CompanionModeRemovesUnrelatedProxyTools()
    {
        const string ownerKey = "webauth:companion-tool-scope-test";
        using var proxy = new HeirowLlm("127.0.0.1", 1234, 24424, 24426);
        ...
        permissions.CompanionScreenView = true;
        permissions.CompanionApplicationLaunch = true;
        permissions.CompanionApplicationControl = true;
        proxy.SaveChatClientPermissionsDiagnostics(permissions);
        MethodInfo? addTools = typeof(HeirowLlm).GetMethod(
            "AddCompanionTools",
            BindingFlags.Instance | BindingFlags.NonPublic);
        MethodInfo? getPermissions = typeof(HeirowLlm).GetMethod(
            "GetChatPermissions",
            BindingFlags.Instance | BindingFlags.NonPublic,
            binder: null,
            types: [typeof(string)],
            modifiers: null);
        Assert.IsNotNull(addTools);
        Assert.IsNotNull(getPermissions);

        const string request = """
        {
          "messages":[{"role":"user","content":"Open Notepad and type test complete."}],
          "tools":[
            {"type":"function","function":{"name":"goal_checkpoint"}},
            {"type":"function","function":{"name":"vs_list_files"}},
            {"type":"function","function":{"name":"run_command_in_terminal"}},
            {"type":"function","function":{"name":"terminal_process_status"}},
            {"type":"function","function":{"name":"browser_open"}}
          ]
        }
        """;

        object storedPermissions = getPermissions.Invoke(proxy, [ownerKey])!;
        string scoped = (string)addTools.Invoke(proxy, [request, storedPermissions, ownerKey])!;

        using JsonDocument document = JsonDocument.Parse(scoped);
        string[] names = document.RootElement.GetProperty("tools").EnumerateArray()
            .Select(tool => tool.GetProperty("function").GetProperty("name").GetString() ?? "")
            .ToArray();
        CollectionAssert.AreEqual(new[] { "companion_action" }, names);
        StringAssert.Contains(scoped, "Never use VS, terminal process, or browser tools");
    }
    ...
    public void CompanionDesktopControlRaisesAndClearsSafetyStateAroundInput()
    {
        ...
        using var proxy = new HeirowLlm("127.0.0.1", 1234, 24434, 24436);
        ChatClientPermissionSnapshot permissions = proxy.GetChatClientPermissionsDiagnostics(ownerKey);
        permissions.CompanionEnabled = true;
        ...
        MethodInfo? execute = typeof(HeirowLlm).GetMethod(
    ...
    [TestMethod]
    public void CompanionKeyboardShortcutIsAFirstClassApplicationControlAction()
    {
        const string ownerKey = "webauth:companion-shortcut-test";
        using var proxy = new HeirowLlm("127.0.0.1", 1234, 24444, 24446);
        ChatClientPermissionSnapshot permissions = proxy.GetChatClientPermissionsDiagnostics(ownerKey);
        permissions.CompanionEnabled = true;
        permissions.CompanionApplicationControl = true;
        proxy.SaveChatClientPermissionsDiagnostics(permissions);
        ...
        string received = "";
        proxy.CompanionControlStateChanged += states.Add;
        proxy.CompanionInput = json => received = json;
        MethodInfo? execute = typeof(HeirowLlm).GetMethod(
            "ExecuteCompanionToolAction",
            BindingFlags.Instance | BindingFlags.NonPublic);
        Assert.IsNotNull(execute);

        string result = (string)execute.Invoke(proxy, [ownerKey, "{\"type\":\"shortcut\",\"value\":\"ALT+F\"}", false])!;

        StringAssert.Contains(received, "ALT+F");
        CollectionAssert.AreEqual(new[] { true, false }, states);
        StringAssert.Contains(result, "\"ok\":true");
    }

    [TestMethod]
    public void FreeServerNeverCalculatesAServiceFee()
    {
        MethodInfo? calculate = typeof(HeirowLlm).GetMethod(
            "CalculateChatServiceFeeUsd",
            BindingFlags.Instance | BindingFlags.NonPublic);
        Assert.IsNotNull(calculate);

        using var proxy = new HeirowLlm("127.0.0.1", 1234, 24454, 24456);
        Assert.AreEqual(0d, (double)calculate.Invoke(proxy, [false, 2.64d, 0.015d])!);
        Assert.AreEqual(0d, (double)calculate.Invoke(proxy, [true, 0d, 0.015d])!);
        Assert.AreEqual(0.0396d, (double)calculate.Invoke(proxy, [true, 2.64d, 0.015d])!, 0.0000001d);
    }
    ...
    {
        bool allowed = HeirowLlm.TryValidateBrowserDestination(new Uri(value), allowPrivateNetwork: false, out string reason);
    ...
    {
        bool allowed = HeirowLlm.TryValidateBrowserDestination(new Uri("https://8.8.8.8/"), allowPrivateNetwork: false, out string reason);
    ...
    {
        bool allowed = HeirowLlm.TryValidateBrowserDestination(new Uri("https://user:password@8.8.8.8/"), allowPrivateNetwork: true, out string reason);
    ...
    {
        bool allowed = HeirowLlm.TryValidateBrowserDestination(new Uri("http://127.0.0.1:8080/"), allowPrivateNetwork: true, out string reason);
    ...
    {
        using var proxy = new HeirowLlm("127.0.0.1", 1234, 21434, 21436)
        {
    ...
    {
        using var proxy = new HeirowLlm("127.0.0.1", 1234, 22434, 22436)
        {
    ...
    {
        using var proxy = new HeirowLlm("127.0.0.1", 1234, 23434, 23436);
    ...
    [TestMethod]
    public void WebChatCarriesEarlierImagePixelsIntoAReferencingFollowUp()
    {
        using var proxy = new HeirowLlm("127.0.0.1", 1234, 23444, 23446);
        const string imageDataUrl = "data:image/jpeg;base64,/9j/4AAQSkZJRg==";
        string requestBody = $$"""
        {
          "model": "vision-model",
          "messages": [
          {
            "role": "user",
            "content": [
              { "type": "text", "text": "does this look broken to you?" },
              { "type": "image_url", "image_url": { "url": "{{imageDataUrl}}" } }
            ]
          },
          {
          ...
          {
            "role": "user",
            "content": "well, is it broken?"
          }]
        }
        """;

        foreach (string methodName in new[] { "BuildChatUiCompletionRequestJson", "BuildChatUiNativeChatRequestJson" })
        {
            MethodInfo? buildRequest = typeof(HeirowLlm).GetMethod(methodName, BindingFlags.Instance | BindingFlags.NonPublic);
            Assert.IsNotNull(buildRequest);
            object?[] arguments = methodName == "BuildChatUiCompletionRequestJson"
                ? [requestBody, true, null, null, null, false]
                : [requestBody, null, null, null, false];
            string routedJson = (string)buildRequest.Invoke(proxy, arguments)!;
            using JsonDocument document = JsonDocument.Parse(routedJson);
            JsonElement messages = document.RootElement.GetProperty("messages");
            JsonElement[] userMessages = messages.EnumerateArray()
                .Where(message => message.GetProperty("role").GetString() == "user")
                .ToArray();

            Assert.AreEqual(1, userMessages.Length, methodName);
            StringAssert.Contains(userMessages[0].GetProperty("content").GetRawText(), imageDataUrl, methodName);
            StringAssert.Contains(userMessages[0].GetProperty("content").GetRawText(), "well, is it broken?", methodName);
            LlmChatRequest parsed = LlmChatRequest.FromJson(document.RootElement);
            Assert.IsTrue(parsed.Messages.Last().HasImageContent, methodName);
            Assert.IsTrue(messages.EnumerateArray().Any(message =>
                message.GetProperty("role").GetString() == "system" &&
                (message.GetProperty("content").GetString() ?? "").Contains("exact earlier image pixels", StringComparison.Ordinal)), methodName);
            Assert.IsTrue(messages.EnumerateArray().Any(message =>
                message.GetProperty("role").GetString() == "system" &&
                (message.GetProperty("content").GetString() ?? "").Contains("surrounding Web Chat interface is not part of the attached image", StringComparison.Ordinal)), methodName);
            Assert.IsFalse(routedJson.Contains("I cannot inspect the image", StringComparison.OrdinalIgnoreCase), methodName);
        }
    }
    ...
    public void WebChatDreamManagementPersistsPermissionsAndProtectsDirtySettings()
    {
        ...
        StringAssert.Contains(html, ".dream-scroll-body.thresholds-expanded");
        StringAssert.Contains(html, "grid-auto-rows: max-content");
        StringAssert.Contains(html, "min-height: max-content");
        StringAssert.Contains(html, "scrollBody?.classList.toggle('thresholds-expanded',!open)");
        StringAssert.Contains(html, "settingsScroll.scrollTo({top:Math.max(0,button.offsetTop-8)");
