...
Enrollment generates a `.jackllm-recovery` file and defaults its save location to the user's Documents folder. The complete file is password protected with an Argon2id-derived key and AES-256-GCM; the recovery key and portable envelope never appear as plaintext in the saved file. By default, the workstation password used during enrollment protects the file. Store it offline or in a protected vault. Recovery and HWID rebinding require the file password, elevation, and Windows Hello, and create a new TPM-bound key on the replacement PC.
