...
- Tracks remote session files and managed local roots through the existing heirowLLM file/session features.

Migration plan:

1. Introduce `heirowLLMSandboxService` in the GUI project that creates one `SandboxSession` per chat/client/session.
...
- Tracks published heirowLLM server rows, owner data, health, shell relay metadata, and active relay session counts.

The master list should add explicit sessions in two layers: web/account sessions and heirowLLM usage sessions.
...
| Table | Key Fields |
|---|---|
| `heirowLLMWebSessions` | `Id`, `AccountUserName`, `AccessTokenHash`, `RememberTokenHash`, `UserAgentHash`, `IpHash`, `CreatedUtc`, `LastSeenUtc`, `ExpiresUtc`, `RevokedUtc`, `ConsentVersion`, `SandboxSessionId`, `CapabilitiesJson` |
| `heirowLLMWebSessionEvents` | `Id`, `SessionId`, `EventType`, `Detail`, `IpHash`, `CreatedUtc` |

Endpoint changes:

| Endpoint | Change |
|---|---|
| `POST /api/web-auth/login` | Create `heirowLLMWebSessions` row, create sandbox session, issue cookie/token pointing to row. |
...
| Table | Key Fields |
|---|---|
| `heirowLLMUsageSessions` | `Id`, `WebSessionId`, `AccountUserName`, `ServerId`, `OwnerUserName`, `RelayId`, `Status`, `StartedUtc`, `LastSeenUtc`, `ClosedUtc`, `TokenBudget`, `TokensUsed`, `PaymentState`, `SandboxSessionId`, `LaunchUrl`, `MetadataJson` |
| `heirowLLMUsageSessionEvents` | `Id`, `UsageSessionId`, `EventType`, `Detail`, `CreatedUtc` |
| `heirowLLMUsageSessionFiles` | `Id`, `UsageSessionId`, `SandboxFileId`, `Name`, `Kind`, `SizeBytes`, `Hash`, `CreatedUtc`, `ExpiresUtc` |

New endpoint examples:

| Endpoint | Purpose |
|---|---|
| `POST /api/heirowllm/sessions` | Create a usage session for a selected server and return launch/proxy info. |
| `GET /api/heirowllm/sessions/current` | List active sessions for the current signed-in user. |
| `POST /api/heirowllm/sessions/{id}/heartbeat` | Renew the usage session and update relay activity. |
| `POST /api/heirowllm/sessions/{id}/close` | Close the session, snapshot or discard sandbox files, and release relay capacity. |
| `GET /api/heirowllm/sessions/{id}/files` | Return public-safe session file metadata. |
...
- Shell relay active session counts should be derived from `heirowLLMUsageSessions` where possible, then reconciled with live relay runtime.
...
| heirowLLM remote files | Register current managed roots as sandbox bind mounts, then move new clones into sandbox storage. |
...
| 2 | Add adapters in heirowLLM and Companion while preserving existing tables/URLs. |
...
- SocketJack.com login creates and renews a revocable `heirowLLMWebSessions` row.
- Launching a listed host creates a `heirowLLMUsageSessions` row with heartbeat and close behavior.
- Companion uploads and downloads can complete through sandbox file IDs without exposing local paths.
- heirowLLM remote session files can be listed, cloned, cleaned up, and snapshotted through `ISandboxFileStore`.
- Turning off file permission blocks import/download across GUI and Companion through the same sandbox policy.
- Existing users can still open old Companion shared files and existing heirowLLM file links during the compatibility period.
