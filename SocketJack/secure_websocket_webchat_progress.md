...
| heirowLLM local `/api/web-chat/ws` endpoint | 100% | `####################` | Registered on the heirowLLM chat server and smoke-tested with `/api/health`. |
...
- `dotnet build ..\heirowLLM\heirowLLM.csproj`: passed after closing the running Workstation process.
