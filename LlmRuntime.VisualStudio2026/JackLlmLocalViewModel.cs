...
[DataContract]
internal abstract class JackLlmLocalViewModel : NotifyPropertyChangedObject
{
    private readonly JackLlmWorkstationAuthService authService = new();
    private JackLlmWorkstationAuthState authState;
    private string signInUserName = "";
    private string signInPassword = "";
    private string signInError = "";
    private string workstationStatus = "Sign in to the local JackLLM Workstation.";
    private bool rememberSignIn = true;
    private bool isWorkstationUnavailable = true;
    private bool isSigningIn;

    protected JackLlmLocalViewModel()
    {
    ...
    protected async Task EnsureWorkstationAvailableAsync(CancellationToken cancellationToken)
    {
        ...
        {
            this.ShowLocalSignIn("Sign in with an account created in JackLLM Workstation.");
    ...
    {
        JackLlmWorkstationAuthService.ApplyAuth(request, accessToken, userName);
    }
    ...
    private async Task SignInAsync(object? commandParameter, CancellationToken cancellationToken)
    {
        ...
        this.SignInError = "";
        this.WorkstationStatus = "Signing in to JackLLM Workstation...";
        try
        {
    ...
    private void ApplyAuthenticatedState()
    {
        ...
        this.WorkstationStatus =
            "Signed in to JackLLM Workstation at " + SocketJackLocalWorkstationDiscovery.DefaultEndpoint +
    ...
    private void ShowLocalSignIn(string message)
    {
        ...
        this.WorkstationStatus = "JackLLM Workstation sign-in is required.";
        this.SignInError = message;
    }
}

internal sealed class JackLlmWorkstationAuthService
{
    private const string WorkstationBaseUrl = "http://127.0.0.1:11436";
    private readonly HttpClient httpClient;

    public JackLlmWorkstationAuthService(HttpClient? httpClient = null)
    {
    ...
    public async Task<JackLlmWorkstationAuthState> LoginAsync(
        string userName,
        string password,
        bool remember,
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException("Enter your JackLLM Workstation user name and password.");
        }
        ...
        {
            throw new InvalidOperationException("JackLLM Workstation sign-in failed: " + ExtractError(json, response.StatusCode));
        }
    ...
    public async Task<JackLlmWorkstationAuthState> ValidateAsync(
        JackLlmWorkstationAuthState current,
        CancellationToken cancellationToken)
    {
    ...
    public JackLlmWorkstationAuthState Load()
    {
        try
        {
            ...
            JackLlmStoredAuth? stored = JsonSerializer.Deserialize<JackLlmStoredAuth>(File.ReadAllText(this.AuthFilePath));
    ...
    public void Save(JackLlmWorkstationAuthState state)
    {
...
internal sealed class JackLlmWorkstationAuthState
{
