...
Use supported Hugging Face models and other local LLMs inside Visual Studio Copilot through a local JackLLM Workstation. The extension has no hosted account, remote server directory, or cloud inference requirement. Sign-in is handled only by the JackLLM Workstation running on this PC.
...
Local model
    |
    v
JackLLM Workstation at http://127.0.0.1:11436
    |
...
Build and run [JackLLM Workstation](https://github.com/JackOfFates/SocketJack/tree/master/JackLLM) on the same PC as Visual Studio. The extension checks:
...
6. Sign in with an account created inside JackLLM Workstation.
