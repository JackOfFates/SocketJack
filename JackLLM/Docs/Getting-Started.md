# JackLLM Workstation: Getting Started

## First run

1. Start JACK from the Workstation home tab.
...
## Where JackLLM keeps your data

All portable per-user state is under `%LOCALAPPDATA%\SocketJack\JackLLM`:
...
- `JackLLM.settings.json` and model-manager JSON files - application settings

Copy or move this complete folder when transferring the same Windows user's JackLLM projects and settings. Do not move only `SocketJackDatabase.json`; its encryption is path-bound and JackLLM performs the supported migration.
