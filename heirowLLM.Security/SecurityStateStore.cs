...
public sealed class SecurityStateStore {
    private static readonly byte[] Entropy = Encoding.UTF8.GetBytes("SocketJack.heirowLLM.Workstation.Security.v1");
    ...
    public SecurityStateStore(bool development, string? root = null, string? registryPathOverride = null) {
        ...
        string baseRoot = root ?? (development
            ? Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "heirowLLM", "Security", "Development")
            : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "heirowLLM", "Security"));
    ...
    private RegistryKey? OpenRegistryKey(bool writable) {
        ...
        string path = _registryPathOverride ?? (_development
            ? @"Software\SocketJack\heirowLLM\Security\Development"
            : @"Software\SocketJack\heirowLLM\Security");
