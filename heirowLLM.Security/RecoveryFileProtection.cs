...
public sealed class PasswordProtectedRecoveryFile {
    public string Format { get; set; } = "heirowLLM Password-Protected Recovery File";
...
public static class RecoveryFileProtection {
    private static readonly byte[] AssociatedData = Encoding.UTF8.GetBytes("heirowLLM-Recovery-File-v2");
    ...
    public static PortableRecoveryFile Unprotect(string protectedFile, string password) {
        ...
        try {
            ...
            if (envelope.Version != 2 ||
                !string.Equals(envelope.Format, "heirowLLM Password-Protected Recovery File", StringComparison.Ordinal) ||
                !string.Equals(envelope.Protection, "Argon2id-65536-3-4/AES-256-GCM", StringComparison.Ordinal))
                throw new InvalidDataException("This is not a supported password-protected heirowLLM recovery file.");
            ...
            try {
                ...
                if (recovery.Version != 1 ||
                    !string.Equals(recovery.Format, "heirowLLM Workstation Recovery Key", StringComparison.Ordinal) ||
