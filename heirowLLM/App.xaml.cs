...
public partial class App : System.Windows.Application {
    ...
    private async void OnStartup(object sender, StartupEventArgs e) {
        ...
        if (wineSafeWpf) {
            MessageBox.Show("heirowLLM Workstation now requires TPM-backed Windows Hello and cannot run through Wine.",
        ...
        try {
            ...
            progress.Report(new StartupLoadingProgress(2, "Preparing heirowLLM Workstation...", "Starting the loading surface."));
            await Dispatcher.Yield(DispatcherPriority.ApplicationIdle);

            _mainWindow = await heirowLLM.MainWindow.CreateAsync(startHidden, progress, _startupCancellation.Token);
            _startupCancellation.Token.ThrowIfCancellationRequested();

            progress.Report(new StartupLoadingProgress(100, "heirowLLM Workstation ready", "Opening the workstation."));
            ...
            MessageBox.Show(
                "heirowLLM Workstation failed to start: " + ex.Message,
                "Startup failed",
                MessageBoxButton.OK,
    ...
    private static string GetCrashLogPath() {
        string configured = Environment.GetEnvironmentVariable("HEIROWLLM_CRASH_LOG") ?? "";
        if (!string.IsNullOrWhiteSpace(configured))
            return configured.Trim();

        return Path.Combine(HeirowLlmUserData.LogsRoot, "wpf-crash.log");
    }
    ...
    private static bool IsWineSafeWpfMode() {
        string? value = Environment.GetEnvironmentVariable("HEIROWLLM_WINE_SAFE_WPF");
