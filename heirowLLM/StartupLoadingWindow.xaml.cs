...
public partial class StartupLoadingWindow : Window {
    ...
    public void ShowLoadingProgress() {
        ...
        // The loading layout needs room for its 112px logo, two text rows,
        // progress track, footer, margins, and custom title bar. The former
        // 260px cap clipped the detail/progress area during longer startup work.
        MinHeight = 350;
        MaxHeight = 400;
        Height = 370;
        Width = 560;
    ...
    private async Task RefreshAuthenticationStatusAsync() {
        ...
        switch (response.State) {
            ...
            case SecurityStateKind.Locked:
                ConfigureAuthenticationMode(AuthenticationMode.Unlock, "Unlock heirowLLM Workstation",
    ...
    private async Task<SecurityResponse> GetSecurityStatusWithStartupAsync() {
        if (!_securityBrokerPreparedForStartup) {
            ...
            try {
                ...
                return new SecurityResponse {
                    ...
                    Message = "heirowLLM could not refresh the Security Broker: " + ex.GetBaseException().Message,
        ...
        try {
            ...
            return new SecurityResponse {
                State = SecurityStateKind.Error,
                Message = "heirowLLM could not start the Security Broker: " + ex.GetBaseException().Message,
                DevelopmentMode = false
            };
        }
    }
    ...
    private async Task RestartOfficialBrokerForStartupAsync(CancellationToken cancellationToken) {
        ...
        if (current.BrokerCompatibility != 0 &&
            ...
            throw new InvalidOperationException(
                $"The running Security Broker is incompatible (broker {current.BrokerCompatibility}, Workstation {SecurityProtocol.BrokerCompatibility}). Reinstall or rebuild heirowLLM Workstation.");
    ...
    private async Task EnsureOfficialBrokerRunningAsync(CancellationToken cancellationToken) {
        try {
            using var service = new ServiceController("heirowLLMSecurityBroker");
            try {
                service.Refresh();
            ...
            if (!File.Exists(brokerPath)) {
                throw new FileNotFoundException(
                    "The official Release Security Broker is not installed or bundled. Rebuild heirowLLM Workstation in Release mode.",
                    brokerPath);
            }
    ...
    private static string ResolveBundledSecurityBrokerPath() {
        string installedPath = Path.Combine(AppContext.BaseDirectory, "heirowLLM.SecurityBroker.exe");
        if (File.Exists(installedPath))
            return installedPath;

        // Source-tree Release builds place the broker in this subdirectory. The MSI
        // installs the same signed executable beside heirowLLM.exe in Program Files.
        return Path.Combine(AppContext.BaseDirectory, "SecurityBroker", "heirowLLM.SecurityBroker.exe");
    }
    ...
    private async Task EnrollAsync(string password) {
        ...
        GeneratedRecoveryKeyText.Text =
            "Your recovery file has been generated and encrypted. Save it before opening heirowLLM; it will not be generated again.";
        RecoveryKeyPanel.Visibility = Visibility.Visible;
        StartEnrollmentGrantTimer();
        AuthenticationActionButton.Content = "I SAVED IT — OPEN HEIROWLLM";
    ...
    private void SaveRecoveryFileButton_Click(object sender, RoutedEventArgs e) {
        ...
        try {
            ...
            if (!string.Equals(Path.GetExtension(path), ".heirowllm-recovery", StringComparison.OrdinalIgnoreCase))
                path += ".heirowllm-recovery";
    ...
    private void BrowseRecoveryOpenPathButton_Click(object sender, RoutedEventArgs e) {
        var dialog = new OpenFileDialog {
            Title = "Open password-protected heirowLLM recovery file",
            InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments),
            Filter = "heirowLLM recovery file (*.heirowllm-recovery)|*.heirowllm-recovery",
    ...
    private static SaveFileDialog CreateRecoverySaveDialog(string currentPath) {
        ...
        string fileName = "heirowLLM-Workstation-Recovery.heirowllm-recovery";
        ...
        return new SaveFileDialog {
            Title = "Save password-protected heirowLLM recovery file",
            InitialDirectory = initialDirectory,
            FileName = fileName,
            DefaultExt = ".heirowllm-recovery",
            Filter = "heirowLLM recovery file (*.heirowllm-recovery)|*.heirowllm-recovery",
            AddExtension = true,
    ...
    private static string GetDefaultRecoveryFilePath() {
        ...
        return Path.Combine(documents, "heirowLLM-Workstation-Recovery.heirowllm-recovery");
    }
    ...
    private async Task<WindowsHelloProof> RunWindowsHelloAsync(Func<Task<WindowsHelloProof>> request) {
        ...
        AuthenticationDetailText.Text = "Opening Windows Hello. heirowLLM will return when verification finishes.";
    ...
    private void SetAuthenticationBusy(bool busy) {
        ...
        else if (_pendingEnrollmentGrant != null) AuthenticationActionButton.Content = "I SAVED IT — OPEN HEIROWLLM";
    ...
    public void UpdateProgress(StartupLoadingProgress progress) {
        ...
        StatusText.Text = string.IsNullOrWhiteSpace(progress.Message)
            ? "Preparing heirowLLM Workstation..."
            : progress.Message;
    ...
    private bool TryRequestCancelStartup() {
        ...
        MessageBoxResult result = MessageBox.Show(
            this,
            "Cancel opening heirowLLM Workstation?",
            "Cancel startup",
