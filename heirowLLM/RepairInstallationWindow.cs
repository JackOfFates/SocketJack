...
internal sealed class RepairInstallationWindow : Window {
    ...
    private void FinishAndRestart() {
        try {
            string exePath = Environment.ProcessPath ?? Path.Combine(AppContext.BaseDirectory, "heirowLLM.exe");
    ...
    private static string BuildFailureDetail(Exception ex) {
        ...
        return "heirowLLM could not load a required install file: " + baseException.Message;
    }
